...
public class SSOAgentRequestResolverTest {
    ...
    public void init() throws SSOException {
        agentConfiguration = new SSOAgentConfiguration(null);
    ...
    @Test(description = "Tests for a request URI which appends the post fix to be recognized as a AuthnRequest")
    public void testSAMLAuthnRequestURL() {
        //  mocks a Request object
        Request request = mock(Request.class);
        when(request.getRequestURI()).thenReturn(TestConstants.AUTHN_REQ_URI);

        SSOAgentRequestResolver resolver = new SSOAgentRequestResolver(request, agentConfiguration);
        Assert.assertTrue(resolver.isSAML2AuthnRequestURL());
    }

    @Test(description = "Tests for a request URI which appends a post fix not recognized as request URL post fix")
    public void testNonAuthnRequestURL() {
        //  mocks a Request object
        Request request = mock(Request.class);
        when(request.getRequestURI()).thenReturn(TestConstants.NON_AUTHN_REQ_URI);

        SSOAgentRequestResolver resolver = new SSOAgentRequestResolver(request, agentConfiguration);
        Assert.assertFalse(resolver.isSAML2AuthnRequestURL());
    }
